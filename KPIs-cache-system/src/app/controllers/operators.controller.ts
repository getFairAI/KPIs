...
router.get('/revenue?:operatorAddress', async (request, response) => {
  try {
    ...
    const [ result ] = await ACTIVE_OPERATORS_MODEL.aggregate([
      ...
      { $group: { _id: null, totalReceived: { $first: '$totalReceived' }, totalSent: { $sum: '$operatorSentTransfers.amount' }, maxReceived: { $first: '$maxReceived'}, maxSent: { $max: '$operatorSentTransfers.amount'}, operatorEvmAddress: { $first: '$operatorEvmAddress' } } },
