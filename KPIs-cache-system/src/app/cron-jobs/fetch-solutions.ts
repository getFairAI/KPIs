    ...
    if (results?.transactions?.edges?.length) {
      ...
      let dataPreparation = results.transactions.edges.map(itemFiltered => {
        return {
          ...
          solutionName: findTag(itemFiltered as transactionEdge, 'solutionName'),
          solutionDescription: findTag(itemFiltered as transactionEdge, 'description'),
          solutionOwner: itemFiltered.node.owner.address,
          rawData: JSON.stringify(itemFiltered.node),
          relatedNewSolutionRequest: null,
          originalSolutionRequest: findTag(itemFiltered as transactionEdge, 'solutionRequestId'),
          output: findTag(itemFiltered as transactionEdge, 'output'),
          outputConfiguration: findTag(itemFiltered as transactionEdge, 'outputConfiguration'),
          rewardsAddress: findTag(itemFiltered as transactionEdge, 'rewardsEvmAddress'),
          timestamp: findTag(itemFiltered as transactionEdge, 'unixTime'),
          allowFiles: findTag(itemFiltered as transactionEdge, 'allowFiles'),
          allowText: findTag(itemFiltered as transactionEdge, 'allowText'),
          contractAddress: findTag(itemFiltered as transactionEdge, 'contractSrc'),
        };
      });
