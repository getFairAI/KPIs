...
import { graphQLarweave, TAG_NAMES, constants, QUERY_LIMIT_ARWEAVE, startBlockArweave } from '../config/api.config.js';
...
export const fetchSolutions = async () => {
    ...
    const latestSavedOnDB = await SOLUTIONS_MODEL.find().lean().sort({ blockHeight: -1 }).limit(1).exec();
    ...
    const queryTags = [
        ...
        { name: TAG_NAMES.operationName, values: [constants.SOLUTION_CREATION] },
    ];
