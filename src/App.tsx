...
import { getMondayDateAndUnixTimeList,
  ...
  AmountUTokenPaymentsPrepareData, } from './kpisFunctions'
...
function App() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if(isExtraChartsEnabled) {
        ...
        setChartKpiPaymentsData(paymentsPrepareData(inferencePaymentTransactionsFiltered, modelCreationTransactionsFiltered, scriptPaymentTransactionsFiltered,activeOperatorsTransactionsFiltered,mondays, 'Payments per week'));
        ...
        setChartKpiNewModelsData(generateChartInfoTxsPerWeek('Week', 'new models per week', 'New models per week', 'New models this week',mapTxByWeekNewModels,mondaysMap));
        ...
        setChartKpiNewScriptsData(generateChartInfoTxsPerWeek('Week', 'new scripts per week', 'New scripts per week', 'New scripts this week',mapTxByWeekNewScripts,mondaysMap));
        ...
        setChartKpiActiveOperatorsData(generateChartInfoTxsPerWeek('Week', 'active operators per week', 'Active operators per week', 'active operators this week',mapTxActiveOperatorsByWeek,mondaysMap));
      }
      ...
      setChartKpiModelsUsedPerWeekData(modelsPerWeekPrepareData(requestsInferenceTransactionsFiltered,uploadModelsTransactionsFilteredExtended,uploadScriptTransactionsFilteredExtended,inferencePaymentTransactionsFiltered,mondays,'Models used per week',false));
      ...
      setChartUPaymentsPerWeek(AmountUTokenPaymentsPrepareData(allPayments,mondays,'$U per week',walletsContent));

      // failed payments per week

      setChartKpiFailedPaymentsModelsPerWeekData(modelsPerWeekPrepareData(requestsInferenceTransactionsFiltered,uploadModelsTransactionsFilteredExtended,uploadScriptTransactionsFilteredExtended,inferencePaymentTransactionsFiltered,mondays,'Failed payments per week by model',true));
  ...
  }, [startDate, endDate, isExtraChartsEnabled, walletsContent]);

  const handleDateChange = (start: Date, end: Date, isExtraEnabled: boolean, walletsContentText: string) => {
